...
const MaterialProposeInfoTable = ({
  selectProposeNumber,
  setSelectProposeNumber,
}: {
  selectProposeNumber: string;
  setSelectProposeNumber: Dispatch<SetStateAction<string>>;
}) => {
  const handleClick = (s: string) => {
    setSelectProposeNumber(s);
  };
  return (
    <div
      className={
        "web:max-w-[1000px] web:rounded-[24px] tablet:min-w-full mobile:max-w-[375px] overflow-scroll mobile:border mobile:rounded-[20px] tablet:rounded-none tablet:border-none "
      }>
      <table
        className={
          ...
          {mockData.map((item, index) => (
            <tr
              ...
              <td className="text-[16px] px-4 border-line_normal border whitespace-nowrap text-center">
                <button
                  className={"underline"}
                  onClick={() => handleClick(item.materialNumber)}>
                  {item.materialNumber}
                </button>
              </td>
              <td className="text-[16px] px-4 border-line_normal border whitespace-nowrap text-center">
...
export default MaterialProposeInfoTable;
